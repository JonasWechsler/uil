...
var parseJSON = function (res, req) {
    ...
    //find name of test
    var next = false;
    for (var z = 0, pagesTextLength = json.formImage.Pages[0].Texts.length; z < pagesTextLength; z++) {
    ...
    //add lines together
    var add = 0;
    for (var i = 0, pagesLength = json.formImage.Pages.length; i < pagesLength; i++) {
        var newFills = []; // per page
        var dead = [];
        var pagesFillsLength = json.formImage.Pages[i].Fills.length;
        for (var j = 0; j < pagesFillsLength; j++) {
            var one = json.formImage.Pages[i].Fills[j];
            var cur = one;
            if (dead.indexOf(j) > -1) {
                continue;
            }
            for (var k = j + 1; k < pagesFillsLength; k++) {
